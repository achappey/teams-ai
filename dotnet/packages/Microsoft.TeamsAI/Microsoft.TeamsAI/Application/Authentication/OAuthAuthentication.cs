...
namespace Microsoft.Teams.AI
{
        ...
        where TState : TurnState, new()
    {
        ...
        public OAuthAuthentication(Application<TState> app, string name, OAuthSettings settings, IStorage? storage)
        {
            ...
            this._messageExtensionAuth = new OAuthMessageExtensionsAuthentication(this._settings.ConnectionName);
            this._botAuthentication = new OAuthBotAuthentication<TState>(app, this._settings, name, storage);
        }
        ...
        {
            TokenResponse tokenResponse = await UserTokenClientWrapper.GetUserTokenAsync(turnContext, this._settings.ConnectionName, "", cancellationToken);

            return tokenResponse != null && tokenResponse.Token != string.Empty ? tokenResponse.Token : null;
        }
        ...
        {
            if (this._botAuthentication == null)
            {
                throw new TeamsAIException("Bot authentication is not initialized.");
            }

            this._botAuthentication.OnUserSignInFailure(handler);
            return this;
        }
        ...
        public IAuthentication<TState> OnUserSignInSuccess(Func<ITurnContext, TState, Task> handler)
        {
            ...
            this._botAuthentication.OnUserSignInSuccess(handler);
            return this;
        }
        ...
        {
            TokenResponse tokenResponse = await UserTokenClientWrapper.GetUserTokenAsync(context, this._settings.ConnectionName, "", cancellationToken);
            if (tokenResponse != null && !string.IsNullOrEmpty(tokenResponse.Token))
            {
                return tokenResponse.Token;
            }

            if ((this._messageExtensionAuth != null && this._messageExtensionAuth.IsValidActivity(context)))
            {
                return await this._messageExtensionAuth.AuthenticateAsync(context);
            }

            return this._botAuthentication != null && this._botAuthentication.IsValidActivity(context)
                ? await this._botAuthentication.AuthenticateAsync(context, state, cancellationToken)
        ...
        {
            this._botAuthentication?.DeleteAuthFlowState(context, state);

            await UserTokenClientWrapper.SignoutUserAsync(context, this._settings.ConnectionName, cancellationToken);
        }
    }
}
