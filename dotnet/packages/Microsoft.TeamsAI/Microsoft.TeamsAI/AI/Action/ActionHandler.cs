...
namespace Microsoft.Teams.AI.AI.Action
{
    ...
    internal sealed class ActionHandler<TState> : IActionHandler<TState> where TState : TurnState
    {
        ...
        private readonly object _containerInstance;

        private readonly Tuple<ActionParameterType, Type>[] _parameterTypes;
        ...
        internal ActionHandler(MethodInfo method, object containerInstance)
        {
            ...
            this._returnType = this._method.ReturnType;
            ...
            foreach (ParameterInfo parameter in parameters)
            {
                ...
                {
                    throw new InvalidOperationException($"Action method parameter should have no more than one parameter attribute. Method name: {this._method.Name}. Parameter name: {parameter.Name}.");
                }
                else
                {
            ...
            this._parameterTypes = parameterTypes.ToArray();
        }
        ...
        public async Task<string> PerformActionAsync(ITurnContext turnContext, TState turnState, object? entities = null, string? action = null, CancellationToken cancellationToken = default)
        {
            ...
            foreach (Tuple<ActionParameterType, Type> parameterType in this._parameterTypes)
            {
                switch (parameterType.Item1)
                {
                    case ActionParameterType.TurnContext:
                        CheckParameterAssignment(this._method, turnContext.GetType(), parameterType.Item2!);
                        parameters.Add(turnContext);
                        break;
                    case ActionParameterType.TurnState:
                        CheckParameterAssignment(this._method, turnState.GetType(), parameterType.Item2!);
                        parameters.Add(turnState);
                        break;
                    case ActionParameterType.Parameters:
                        if (entities != null)
                        {
                            CheckParameterAssignment(this._method, entities.GetType(), parameterType.Item2!);
                        }
                    ...
                    case ActionParameterType.Name:
                        if (action != null)
                        {
                            CheckParameterAssignment(this._method, action.GetType(), parameterType.Item2!);
                        }
            ...
            try
            {
                ...
                return this._returnType == typeof(string)
                    ? (string)result
                    : this._returnType == typeof(Task<string>)
                        ? await (Task<string>)result
                        : this._returnType == typeof(ValueTask<string>)
                                            ? await (ValueTask<string>)result
                                            : throw new InvalidOperationException($"Action method return type should be one of [string, Task<string>, ValueTask<string>]. Method name: {this._method.Name}.");
            }
